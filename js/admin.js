...
if (emailLoginBtn) {
    emailLoginBtn.addEventListener("click", async () => {
        ...
        try {
            ...
            showMessage(
                loginMessage,
                "الإيميل أو كلمة المرور غير صحيحة، أو المستخدم غير مفعل.",
                "error"
            );
        } finally {
...
onAuthStateChanged(auth, async user => {
    ...
    try {
        ...
        if (userManagementPanel) {
            userManagementPanel.style.display =
                isSuperAdmin() ? "block" : "none";
        }

        if (newEventBtn) {
            newEventBtn.style.display =
                isSuperAdmin() ? "inline-block" : "none";
        }
...
async function loadEvents() {
    if (isSuperAdmin()) {
        ...
        const ids = Array.isArray(
            currentUserProfile?.allowedEventIds
        )
            ? currentUserProfile.allowedEventIds
            : [];
...
async function syncSeatLocksForEvent(
    eventId,
    eventBookings
) {
    ...
    for (const lockDoc of lockSnapshot.docs) {
        ...
        const owner = lock.bookingId
            ? activeBookings.get(lock.bookingId)
            : null;
    ...
    for (const booking of activeBookings.values()) {
        ...
        for (const seatNumber of seats) {
            ...
            const existingOwner =
                lockedSeats.get(normalizedSeat);

            // Never overwrite another booking
            if (
                existingOwner &&
                existingOwner !== booking.id
            ) {
                continue;
            }
...
async function loadAdminData() {
    ...
    try {
        ...
        if (eventsAdminList) {
            eventsAdminList.innerHTML = `
                <p class="error">
                    حدث خطأ أثناء تحميل الحفلات.<br>
                    ${escapeHTML(
                        error?.message ||
                        "Unknown error"
                    )}
    ...
    try {
        ...
        if (bookingsList) {
            bookingsList.innerHTML = `
                <p class="error">
                    حدث خطأ أثناء تحميل الحجوزات.<br>
                    ${escapeHTML(
                        error?.message ||
                        "Unknown error"
                    )}
...
function renderEvents() {
    ...
    if (!events.length) {
        eventsAdminList.innerHTML =
            `<div class="empty">لا توجد حفلات مضافة حاليًا.</div>`;
    ...
    eventsAdminList.innerHTML =
        events.map(event => {
            ...
            const managerName =
                event.bookingManagerName ||
                "غير محدد";

            const managerPhone =
                event.bookingManagerPhone ||
                "غير محدد";
    ...
    document
        .querySelectorAll(".toggle-event-btn")
        .forEach(button => {
            button.addEventListener(
                "click",
                () => toggleEvent(button.dataset.id)
            );
        });
...
function renderUserEventAssignments(
    selected = []
) {
    if (!userEventAssignments) return;

    if (!events.length) {
        userEventAssignments.innerHTML =
            "<p>أضف حفلات أولًا.</p>";
...
function renderUsers() {
    ...
    if (!managedUsers.length) {
        usersList.innerHTML =
            '<div class="empty">لا يوجد مستخدمون إضافيون.</div>';
    ...
    document
        .querySelectorAll(".toggle-user-btn")
        .forEach(button => {
            button.addEventListener(
                "click",
                () =>
...
if (eventPosterUrl) {
    eventPosterUrl.addEventListener(
        "input",
        () => {
            const url =
                eventPosterUrl.value.trim();
            ...
            if (!url) {
                ...
                if (posterPreviewImage) {
                    posterPreviewImage.removeAttribute(
                        "src"
                    );
                }
    ...
    eventPosterUrl.addEventListener(
        ...
        () => {
            ...
            if (!isValidPosterUrl(url)) {
                showMessage(
                    eventMessage,
                    "اكتب رابط صورة صحيح يبدأ بـ https:// أو http://.",
                    "error"
                );
            ...
            showMessage(
                eventMessage,
                "تم إدخال رابط الصورة. اضغط حفظ الحفلة.",
                "success"
            );
        }
    );
...
function editEvent(id) {
    ...
    bookingManagerName.value =
        event.bookingManagerName || "";

    bookingManagerPhone.value =
        event.bookingManagerPhone || "";
...
async function saveEvent() {
    ...
    const managerName =
        bookingManagerName.value.trim();

    const managerPhone =
        normalizeEgyptianPhone(
            bookingManagerPhone.value.trim()
        );
    ...
    if (
        ...
        showMessage(
            eventMessage,
            "اكتب رقم مسؤول الحجز المصري صحيحًا من 11 رقم.",
            "error"
        );
    ...
    if (
        poster &&
        !isValidPosterUrl(poster)
    ) {
        showMessage(
            eventMessage,
            "رابط صورة الحفلة غير صحيح. استخدم رابطًا مباشرًا يبدأ بـ https:// أو http://.",
            "error"
        );
...
async function toggleEvent(id) {
    ...
    try {
        ...
        alert(
            "حدث خطأ أثناء تغيير حالة الحفلة."
        );
    }
}
...
async function deleteEvent(id) {
    ...
    if (relatedBookings.length) {
        alert(
            "لا يمكن حذف حفلة بها حجوزات. قم بإيقاف الحجز بدلًا من حذفها."
        );
        return;
    }
        ...
        <div class="booking-card">
            ...
            <div class="booking-actions">
                ...
                ${
                    ...
                    <button
                        type="button"
                        class="confirm-booking-btn success"
                ...
                <button
                    type="button"
                    class="delete-booking-btn danger"
                    data-booking-id="${escapeHTML(
                        booking.id
                    )}"
                    data-event-id="${escapeHTML(
...
async function confirmBooking(
    ...
    if (
        !confirm(
            `هل تريد تأكيد الحجز ${bookingNumber}؟`
        )
    ) {
        return;
    }
...
async function saveBookingEdit() {
    ...
    if (
        ...
        showMessage(
            bookingEditMessage,
            "رقم الموبايل يجب أن يكون 11 رقم ويبدأ بـ 01.",
            "error"
        );
        return;
    }
    ...
    if (!seats.length) {
        showMessage(
            bookingEditMessage,
            "يجب اختيار مقعد واحد على الأقل.",
            "error"
        );
        return;
    }
    ...
    try {
        ...
        if (conflict) {
            showMessage(
                bookingEditMessage,
                "أحد المقاعد المختارة محجوز بالفعل لحجز آخر.",
                "error"
            );
            return;
        }
...
if (deleteBookingBtn) {
    deleteBookingBtn.addEventListener(
        "click",
        async () => {
            const bookingId =
                editBookingId.value;
...
async function cancelBooking(
    ...
    if (
        !confirm(
            `هل أنت متأكد من إلغاء الحجز ${bookingNumber}؟\n\nسيتم تحرير المقاعد.`
        )
    ) {
        return;
    }
    ...
    try {
        ...
        alert(
            "تم إلغاء الحجز وتحرير المقاعد بنجاح."
        );
